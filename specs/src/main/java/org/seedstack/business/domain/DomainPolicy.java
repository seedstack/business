...
/**
 * A policy is used to encapsulate a varying business rule or process in a separate object. Multiple versions of the
 * policy object represent different ways the process can be done.
 *
 * <p>
 * This annotation can be applied to an interface to declare a domain policy. Any class implementing the annotated interface
 * will be registered by the framework as an implementation of this policy.
 * </p>
 *
 * <p>
 * When a policy interface has multiple implementations, it is necessary to differentiate them by using a different
 * {@code javax.inject.Qualifier} annotation on each. This qualifier can then be used at the injection point to specify
