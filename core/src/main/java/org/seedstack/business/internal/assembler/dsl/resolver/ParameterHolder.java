...
class ParameterHolder<D> {
    ...
    <ID> ID uniqueElementForAggregate(D dto, int aggregateIndex) {
        checkNotNull(dto, "DTO cannot be null");
        checkArgument(aggregateIndex >= 0, "Aggregate index must be greater than or equal to zero");
    ...
    boolean isEmpty() {
        return aggregateGetter == null &&
                (aggregateGetters == null || aggregateGetters.length == 0) &&
                isEmptyForAggregateRoot(0);
    }

    private boolean isEmptyForAggregateRoot(int aggregateIndex) {
        checkArgument(aggregateIndex >= 0, "Aggregate index must be greater than or equal to zero");
        return (tupleGetter == null || tupleGetter.length == 0) &&
                (tupleGetters == null || tupleGetters.length == 0 || aggregateIndex >= tupleGetters.length || tupleGetters[aggregateIndex] == null || tupleGetters[aggregateIndex].length == 0);
    }
}
