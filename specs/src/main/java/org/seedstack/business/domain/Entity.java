 ...
 * it. Do not create entities with only getters and setters but add methods with meaningful business names, implementing
 * domain behavior.
 * </p>
 *
 * <p>
...
public interface Entity<ID> {
    ...
    /**
     * As per Domain-Driven Design semantics, entity equality must be computed on its identity only, as returned by the
     * {@link #getId()} method. Two entities from the same class hierarchy (with an inheritance relationship between them)
     * can be checked for equality as they have a comparable identity.
     *
     * <p>
     * When implementing this method for entities, the semantics above must be respected in addition to the semantics of
     * {@link Object#equals(Object)}.
     * </p>
     *
     * @param other other object
     * @return true if the other object is an entity of the same class hierarchy as this entity and has an identity equal
     * to this entity identity.
     */
    boolean equals(Object other);

    /**
     * As per Domain-Driven Design semantics, the hash code of an entity must be computed on its identity only, as return
     * by as returned by the {@link #getId()} method.
     *
