...
class FinderModule extends AbstractModule {
    private static final Logger LOGGER = LoggerFactory.getLogger(FinderModule.class);
    private final Map<Key<?>, Class<?>> bindings;
    private final Collection<BindingStrategy> bindingStrategies;

    FinderModule(Map<Key<?>, Class<?>> bindings, Collection<BindingStrategy> bindingStrategies) {
