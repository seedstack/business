...
public class AssemblerRegistryIT {
    ...
    public void testAssemblerOfTuple() {
        Class<? extends AggregateRoot<?>>[] aggregateRootTuple = new Class[]{org.seedstack.business.fixtures.assembler.customer.Order.class, Customer.class};
        Assembler<?, ?> assembler = registry.tupleAssemblerOf(aggregateRootTuple, Recipe.class, ModelMapper.class);
        Assertions.assertThat(assembler).isNotNull();
    }

    @Test
    public void testAssemblerOfWithAutomaticAssembler() {
        Assembler<?, ?> assembler = registry.assemblerOf(org.seedstack.business.fixtures.assembler.customer.Order.class
                , org.seedstack.business.fixtures.assembler.customer.OrderDto.class);
        Assertions.assertThat(assembler).isNotNull();
        Assertions.assertThat(assembler).isInstanceOf(ModelMapperAssembler.class);
