...
public class BusinessIT {
    ...
    public void basicFunctionalTests() throws ActivationException {
        ...
        Product loadedProduct1 = holder.productRepo.get(new ProductId((short) 1, "ean13-1"))
                .get();
        Product loadedProduct2 = holder.productRepo.get(new ProductId((short) 2, (short) 5))
                .get();

        assertThat(loadedProduct1.getId()
                .getStoreId()).isEqualTo((short) 1);
        assertThat(loadedProduct1.getId()
                .getProductCode()).isEqualTo("ean13-1");
        ...
        assertThat(loadedProduct2.getId()
                .getStoreId()).isEqualTo((short) 2);
        assertThat(loadedProduct2.getId()
                .getProductCode()).isEqualTo("ean13-5");
        ...
        Order load2_ = holder.orderRepo.get(new OrderId("commande01"))
                .get();
        assertThat(load2_).isNotNull();

        Customer custLoadedKey1 = holder.customerRepo.get(new CustomerId("key1"))
                .get();
        assertThat(custLoadedKey1).isNotNull();

        assertThat(load2_.getId()
                .getValue()).isEqualTo(new OrderId("commande01").getValue());
        assertThat(load2_.getCustomerId()).isEqualTo(new CustomerId("2"));

        CustomerId entityId = new CustomerId("key1");
        Customer load = holder.customerRepo.get(entityId)
                .get();
        assertThat(load).isNotNull();

        entityId = new CustomerId("2");
        load = holder.customerRepo.get(entityId)
                .get();
        assertThat(load).isNotNull();
