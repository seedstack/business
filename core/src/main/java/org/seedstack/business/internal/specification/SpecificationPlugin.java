...
public class SpecificationPlugin extends AbstractSeedPlugin {
    ...
    public InitState initialize(InitContext initContext) {
        streamClasses(initContext, BusinessSpecifications.SPECIFICATION_TRANSLATOR, SpecificationTranslator.class).forEach(specificationTranslatorClasses::add);
        LOGGER.debug("Specification translator classes => {}", specificationTranslatorClasses);

        streamClasses(initContext, BusinessSpecifications.SPECIFICATION_CONVERTER, SpecificationConverter.class).forEach(specificationConverterClasses::add);
    ...
    public Object nativeUnitModule() {
        return new SpecificationModule(
                specificationTranslatorClasses,
